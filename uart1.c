...
// Send hexadecimal number as ASCII
...
// Send a byte in hexadecimal format
void uart_hex_byte(unsigned int n)
{
    unsigned int nibble;
    if (n < 16)
    {
    ...
    for (int c = 4; c >= 0; c = c - 4)
    {
        nibble = (n >> c) & 0xF;
        nibble += (nibble > 9) ? (-10 + 'A') : '0';
        uart_sendc(nibble);
    }
}